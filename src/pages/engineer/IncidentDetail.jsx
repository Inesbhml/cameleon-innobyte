...
export const IncidentDetail = () => {
  ...
  const workflowType = incident?.workflow?.split(' ')[0] || 'Phishing';
  const rawSteps = mockWorkflows[workflowType] || mockWorkflows['Phishing'] || [];
  const [steps, setSteps] = useState([]);

  useEffect(() => { setSteps(rawSteps.map(s => ({ ...s }))); }, [workflowType]);
